"""
Data-Analyst Telegram Bot — starter code (educational).
Stack: FastAPI (web server + webhook + serves the log) + OpenAI-style LLM API.

Environment variables you must set on your host:
  TELEGRAM_TOKEN  - from @BotFather
  LLM_API_KEY     - your LLM provider key
  PUBLIC_URL      - your deployed base URL, e.g. https://myapp.onrender.com

Run locally:  uvicorn bot:app --port 8000
...
LLM_ENDPOINT = "https://aipipe.org/openai/v1/chat/completions"  # or your provider
...
def run_python(code: str) -> str:
    """Execute Python and return stdout (or the traceback). The LLM writes
    the code; pandas etc. are available because they're in requirements.txt."""
    buf = io.StringIO()
    try:
...
Use the run_python and fetch_url tools to actually compute answers from data
(inline data in the message, or public datasets like MOSPI).
When finished, reply with EXACTLY one JSON object matching the shape the
question asks for, e.g. {"answer": {...}} — no prose, no markdown fences.
Do not invent the log_url; the server fills it in."""
...
async def webhook(request: Request):
    ...
    if text and chat.get("id"):
        reply = await run_agent(chat["id"], text)
        await send_telegram(chat["id"], reply)
    return {"ok": True}
